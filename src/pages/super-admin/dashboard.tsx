...
const SuperAdminDashboard = () => {
  ...
  const fetchDashboardData = async () => {
    try {
      ...
      // Fetch stats
      const [orgsResult, usersResult, subscriptionsResult] = await Promise.all([supabase.from("organisations").select("*", {
        count: "exact"
  ...
  return <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
        ...
        <StatCard title="Active Subscriptions" value={stats.activeSubscriptions} icon={TrendingUp} description="Paying customers" />
        <StatCard title="Monthly Revenue" value={`₹${stats.monthlyRevenue.toLocaleString()}`} icon={DollarSign} trend={{
        value: "+15%",
        positive: true
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <Card>
          ...
          <CardContent>
            {loading ? <div className="text-center py-4 text-muted-foreground">Loading...</div> : recentOrgs.length === 0 ? <div className="text-center py-4 text-muted-foreground">No organizations yet</div> : <div className="space-y-3">
                {recentOrgs.map(org => <div key={org.id} className="flex items-center justify-between p-3 rounded-lg border hover:bg-muted/50 transition-colors">
        ...
        <Card>
          ...
          <CardContent>
            {loading ? <div className="text-center py-4 text-muted-foreground">Loading...</div> : recentUsers.length === 0 ? <div className="text-center py-4 text-muted-foreground">No users yet</div> : <div className="space-y-3">
                {recentUsers.map(user => <div key={user.id} className="flex items-center justify-between p-3 rounded-lg border hover:bg-muted/50 transition-colors">
                    <div className="flex-1">
      ...
      <Card>
        ...
        <CardContent>
          {loading ? <div className="text-center py-4 text-muted-foreground">Loading...</div> : recentLogs.length === 0 ? <div className="text-center py-4 text-muted-foreground">No activity logs yet</div> : <div className="space-y-2">
              {recentLogs.map(log => <div key={log.id} className="flex items-start gap-3 p-3 rounded-lg border hover:bg-muted/50 transition-colors">
                  <div className="mt-0.5">
                    {getActionIcon(log.action_type)}
                  </div>
                  <div className="flex-1 min-w-0">
                    <p className="text-sm font-medium">{log.action_type.replace(/_/g, ' ')}</p>
                    {log.metadata && <p className="text-sm text-muted-foreground truncate">
