...
export function OrgAdminSidebar() {
  ...
  const isActive = (path: string) => {
    if (path === "/dashboard/org-admin") {
      return currentPath === "/dashboard/org-admin";
    }
  ...
  return (
    ...
    >
      ...
      <div
        className="flex items-center border-b border-border pl-12 pr-2"
        style={{ height: "52px" }}
      >
        <div
        ...
        <nav className="space-y-1 px-2">
          {navItems.map((item) => {
            ...
            const menuButton = (
              <NavLink
                to={item.url}
                end={item.url === "/dashboard/org-admin"}
